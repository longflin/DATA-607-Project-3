...
#Connect to database
con <- dbConnect(SQLite(), "ds_jobs_Summary.db")
...
## Comparing Jobs Requiring Skills shared between our two data sources.

```{r}
#Add year columns to final data frames for both sources
df1_final <- ds_job_title %>% mutate(Year = "2020")
...
plot_ready_data <- skill_counts %>%
  ...
  # Remove any skills not categorized in our clusters
