...
ggplot(viz_data, aes(x = reorder(Skills, Frequency), y = Frequency, fill = Cluster)) +
  geom_bar(stat = "identity", alpha = 0.85) +
  geom_text(aes(label = Frequency),          # <-- add this
            hjust = -0.2, 
            size = 3.5, 
            color = "gray30") +
  coord_flip() +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.15))) +  # <-- add this
  labs(
